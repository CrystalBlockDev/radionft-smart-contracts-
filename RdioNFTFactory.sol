...
contract RadioNFTFactory is Ownable,ERC1155Receiver {
    ...
    struct RoyaltyInfo {
        ...
        uint256 radioTeamAmount;
        address radioTeamAddress;
    ...
    function createSaleReal(string memory _tokenHash, uint _interval, uint _price, uint8 _kind) public returns (bool) {
        ...
        emit CreateSaleReal(_tokenHash, _interval, _price, _kind);
        return true;
    }
    ...
    function endBidReal(string memory _tokenHash) internal returns(BidInfo memory bidInfos, RoyaltyInfo memory royaltyInfos){
        ...
        royaltyInfo.sellerAmount = saleInfo.maxBid - royaltyInfo.artistAmount - royaltyInfo.radioTeamAmount - royaltyInfo.devTeamAmount;
        ...
        if(royaltyInfo.radioTeamAmount > 0) {
            customizedTransfer(payable(royaltyInfo.radioTeamAddress), royaltyInfo.radioTeamAmount, saleInfo.kindOfCoin);
        }
    ...
    function burnNFT(string memory tokenHash) external onlyNFTOwner(tokenHash){
        mkNFT.burn(msg.sender, _getNFTId[tokenHash], 1);
